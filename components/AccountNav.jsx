import { Link } from '../renderer/Link'
import { useAuth0 } from "@auth0/auth0-react";
import { Button } from './button.jsx';
import { LogOut } from 'lucide-react';
import { usePageContext } from '../renderer/usePageContext'

const AccountNav = () => {

  const { logout } = useAuth0();

  const pageContext = usePageContext()
  const { urlPathname } = pageContext

  const linkClases = (type = null) => {
    let classes =
      'flex justify-center mx-10 md:mx-0 gap-1 py-2 px-6 rounded-full';
    if (type === urlPathname) {
      classes += ' bg-pink text-white';
    } else {
      classes += ' bg-gray-200';
    }
    return classes;
  };
  return (
    <nav className="mt-24 mb-8 flex w-full flex-col justify-center gap-2 p-8 md:flex-row md:p-0">
      <Link className={linkClases('/account/bookings')} href={`/account/bookings`}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="h-6 w-6"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"
          />
        </svg>
        My Bookings
      </Link>
      <Link className={linkClases('/account')} href={'/account'}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="h-6 w-6"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M8.25 21v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21m0 0h4.5V3.545M12.75 21h7.5V10.75M2.25 21h1.5m18 0h-18M2.25 9l4.5-1.636M18.75 3l-1.5.545m0 6.205l3 1m1.5.5l-1.5-.5M6.75 7.364V3h-3v18m3-13.636l10.5-3.819"
          />
        </svg>
        My Places
      </Link>
      <Button variant="secondary" className={linkClases('places')} onClick={() => logout()}>
        <LogOut className="mr-2 h-4 w-4"/>
        Logout
      </Button>
    </nav>
  );
};

export default AccountNav;
